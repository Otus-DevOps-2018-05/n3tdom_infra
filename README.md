...
### Задание со *
- Вынести роль db в отдельный репозиторий, подключить TravisCI (результат = бейдж билда, слак чат)
TODO: Не выполнялось
...
- в gce.ini задан фильтр на хосты prod/stage окружения - вызов инвентаря выдает только хосты соответсвующего окружения:
ansible-playbook -i environments/stage/gce.py playbooks/site.yml --diff --check

### Задание со ** - TravisCI + packer,terraform,ansible
TODO: Не выполнялось
